...
public class EditProcedure extends AbstractDBTreeContextAction {
    ...
    public String getText() {
        ...
        return Messages.getString("mssql.Actions.EditProcedure");
    }
    ...
    public boolean isAvailable() {
    	...
    	if( _selectedNodes.length == 1 ) {
    		...
    		if (_selectedNodes[0] instanceof ProcedureNode) {
    			return true;
    		}
    	}
	...
	public void run() {
		...
		try {
			...
			pStmt = connection.prepareStatement("SELECT sc.text FROM " + _selectedNodes[0].getSchemaOrCatalogName() +
					...
					".dbo.syscomments sc ON so.id=sc.id WHERE so.type='P' AND so.name=?");
            ...
            editorPart.setText(query.toString().replaceAll("\\b(?i:create\\s*proc\\w*)\\b", "alter procedure "));
