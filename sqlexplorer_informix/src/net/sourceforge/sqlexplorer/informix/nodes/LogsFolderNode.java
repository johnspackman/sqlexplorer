...
public class LogsFolderNode extends AbstractFolderNode {

    public LogsFolderNode(INode parent, String name, MetaDataSession session) {
    	super(parent, name, session, "logs_folder");
    }
	...
	public void loadChildren() {
        ...
        try {
            ...
            rs = stmt.executeQuery("SELECT number FROM sysmaster:syslogs ORDER BY number");

            while (rs.next()) {
                addChildNode(new LogNode(this, "Log #"+rs.getString(1), _session, idx++, rs.getString(1)));
            }
        } 
        catch (Exception e) {
