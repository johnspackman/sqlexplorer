...
public class CreatePackageBodyEdit extends Action {

	SessionTreeNode sessionNode;
	IDbModel node;
	public CreatePackageBodyEdit(SessionTreeNode sessionTreeNode, IDbModel node) {
	...
	public String getText(){
		return "Create a new Package Body..."; //$NON-NLS-1$
	}
	public void run(){
		String owner=((SchemaNode)(((PackageBodyTypeNode)node).getParent())).toString();
		String separator= System.getProperty("line.separator"); //$NON-NLS-1$
		String txt="CREATE OR REPLACE PACKAGE BODY "+owner+".<package> AS"+separator
             + "--"+separator
             + "-- NAME        : "+separator
		...
		ObjectEditor oe=new ObjectEditor(null,store,sessionNode.getDictionary(),txt,"Creating a package body",conn,"PACKAGE BODY",owner,null);
		oe.open();
