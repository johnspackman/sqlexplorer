 ...
 */
public class DBLinkNode implements IDbModel {
	static final Object[] keys={"UserName","Host","Created"};
	...
	public DBLinkNode(IDbModel s,String name,SQLConnection conn){
		this.conn=conn;
		parent=s;
		txt=name;
	}
	...
	private HashMap getInfo(){
		final String sql="SELECT  username, host, created FROM sys.all_db_links where owner=? and db_link=?";
		...
		try{
			...
			if(rs.next()){
				...
				map.put(keys[2],rs.getDate(3).toString());
			}
			rs.close();
			ps.close();
		}catch(Throwable e){
