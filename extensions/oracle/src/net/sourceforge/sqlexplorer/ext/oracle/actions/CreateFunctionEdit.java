...
public class CreateFunctionEdit extends Action {
	...
	public String getText(){
		return "Create a new Function..."; //$NON-NLS-1$
	}
	public void run(){
		String owner=((SchemaNode)(((FunctionTypeNode)node).getParent())).toString();
		String separator= System.getProperty("line.separator"); //$NON-NLS-1$
		String txt="CREATE OR REPLACE FUNCTION "+owner+".<function> ("+separator
		+ "  parameter1  IN  VARCHAR2"+separator
		+ ")"+separator
		+ "RETURN NUMBER AS"+separator
             + "--"+separator
             + "-- NAME        : "+separator
             ...
             +"BEGIN"+separator
             +" return 1;--do something"+separator
             + "END;"+separator;
		IPreferenceStore store=SQLExplorerPlugin.getDefault().getPreferenceStore();
		SQLConnection conn=sessionNode.getConnection();
		ObjectEditor oe=new ObjectEditor(null,store,sessionNode.getDictionary(),txt,"Creating a function ",conn,"FUNCTION",owner,null);
		oe.open();
	}
