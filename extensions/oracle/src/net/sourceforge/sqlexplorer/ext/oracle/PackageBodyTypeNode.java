...
public class PackageBodyTypeNode extends AbstractTypeNode implements IDbModel {
	...
	public void load(){
		...
		try{
			...
			while(rs.next()){
				...
				PackageBodyNode pkbNode=new PackageBodyNode(this,name,conn);
				list.add(pkbNode);
			}
			rs.close();
			ps.close();
	...
	public PackageBodyTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
