...
public class FunctionTypeNode extends AbstractTypeNode implements IDbModel {
	...
	public void load(){
		...
		try{
			...
			while(rs.next()){
				...
				FunctionNode fctNode=new FunctionNode(this,name,conn);
				list.add(fctNode);
			}
			rs.close();
			ps.close();
	...
	public FunctionTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
	};
