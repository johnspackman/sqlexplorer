...
public class JavaTypeNode extends AbstractTypeNode  implements IDbModel {
	public Composite getComposite(DetailManager dm){return null;};
		...
		public void load(){
			...
			try{
				...
				while(rs.next()){
					...
					JavaSourceNode javaNode=new JavaSourceNode(this,name,conn);
					list.add(javaNode);
				}
				rs.close();
				ps.close();
		...
		public JavaTypeNode(IDbModel s,String name,SQLConnection conn){
			parent=s;
			txt=name;
			this.conn=conn;
