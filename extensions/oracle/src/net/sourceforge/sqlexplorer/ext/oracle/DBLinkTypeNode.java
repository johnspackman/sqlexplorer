...
public class DBLinkTypeNode extends AbstractTypeNode implements IDbModel {
	...
	public void load(){
		ResultSet rs=null;
		try{
			final String sql="SELECT db_link "+
						" FROM  sys.all_db_links where owner=?"+
						" ORDER BY db_link";
			...
			while(rs.next()){
				...
				DBLinkNode javaNode=new DBLinkNode(this,name,conn);
				list.add(javaNode);
			}
			rs.close();
			ps.close();
	...
	public DBLinkTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
