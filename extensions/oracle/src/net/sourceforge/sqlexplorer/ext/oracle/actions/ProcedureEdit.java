...
public class ProcedureEdit extends Action {
	...
	IDbModel nd;
		...
		public String getText(){
			return "Procedure Edit"; //$NON-NLS-1$
		}
		public void run(){
		
			try{
				ProcNode pn=(ProcNode)nd;
				final String sql="SELECT text,line FROM sys.all_source where owner=? and name=? and type='PROCEDURE' order by line";
				...
				rs.close();
				stmt.close();
				ObjectEditor oe=new ObjectEditor(null,store,treeNode.getDictionary(),finalResult,"Editing "+pn.toString(),conn,"PROCEDURE",pn.getOwner(),pn.toString());
				oe.open();
			}catch(Throwable e){
				SQLExplorerPlugin.error("Error opening procedure source",e);
			}
		}
