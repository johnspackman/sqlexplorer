...
public class SequenceTypeNode extends AbstractTypeNode implements IDbModel{
	...
	public void load(){
		...
		try{
			...
			while(rs.next()){
				...
				SequenceNode sqNode=new SequenceNode(this,name,conn);
				list.add(sqNode);
			}
			rs.close();
			ps.close();
	...
	public SequenceTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
	};
