 ...
 */
public class JavaSourceNode implements IDbModel {
	...
	static final Object[] keys={"Created","Last DDL Time","TimeStamp","Status"};
		...
		public String getOwner(){
			return ((SchemaNode)((JavaTypeNode)parent).getParent()).toString();
		}
		private String getSource(){
			StringBuffer buf=new StringBuffer();
		
			final String sql="SELECT text FROM sys.all_source where owner=? and name=? and type='JAVA SOURCE' order by line";
			String delimiter=viewer.getTextWidget().getLineDelimiter();
			ResultSet rs=null;
			try{
				PreparedStatement ps=conn.prepareStatement(sql);
				String owner=((SchemaNode)((JavaTypeNode)parent).getParent()).toString();
		...
		private HashMap getInfo(){
			final String sql="SELECT  created,last_ddl_time, timestamp, status FROM sys.all_objects where owner=? and object_type='JAVA SOURCE' and object_name=?";
