...
public class PackageTypeNode extends AbstractTypeNode implements IDbModel {
	...
	public void load(){	
		...
		try{
			...
			while(rs.next()){
				...
				PackageNode pkNode=new PackageNode(this,name,conn);
				list.add(pkNode);
			}
			rs.close();
			ps.close();
	...
	public PackageTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
