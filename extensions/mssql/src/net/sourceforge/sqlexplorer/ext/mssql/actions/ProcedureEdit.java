...
public class ProcedureEdit extends Action {
	...
	IDbModel node;
	public ProcedureEdit(SessionTreeNode sessionTreeNode, IDbModel node) {
	...
	public String getText(){
		return "Edit an existing Procedure...";
	}
	public void run(){
		try{
			ProcNode pn=(ProcNode) node;
			String dbName=((IDbModel)pn.getParent()).getParent().toString();
			final String sql="SELECT sc.text, sc.id, sc.colid FROM " + dbName + 
				"..sysobjects so, " + dbName + 
				"..syscomments sc WHERE so.type='P' AND so.name=? AND sc.id=so.id order by 2,3";
			SQLConnection conn=sessionNode.getConnection();
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, pn.toString());
			...
			rs.close();
			stmt.close();
			SQLEditorInput input = new SQLEditorInput("EDIT PROCEDURE ("+pn.toString()+").sql");
