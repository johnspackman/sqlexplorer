...
public class UDFTypeNode implements IDbModel {
	...
	public Object[] getChildren(){
		if(!loaded){
			ResultSet rs=null;
			try{
				final String sql="SELECT name FROM " + parent.toString() +"..sysobjects WHERE type='FN' and name is not NULL ORDER BY 1";
				PreparedStatement ps=conn.prepareStatement(sql);
				rs=ps.executeQuery();
				while(rs.next()){
					String name=rs.getString(1);
					UDFNode functionNode=new UDFNode(this,name,conn);
	...
	public UDFTypeNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
	};
