...
public class FunctionEdit extends Action {
	...
	IDbModel node;
	public FunctionEdit(SessionTreeNode sessionTreeNode, IDbModel node) {
	...
	public String getText(){
		return "Edit an existing Function...";
	}
	public void run(){
		try{
			UDFNode functionNode=(UDFNode) node;
			String dbName=((IDbModel)functionNode.getParent()).getParent().toString();
			final String sql="SELECT sc.text, sc.id, sc.colid FROM " + dbName + 
				"..sysobjects so, " + dbName + 
				"..syscomments sc WHERE so.type='FN' AND so.name=? AND sc.id=so.id order by 2,3";
			SQLConnection conn=sessionNode.getConnection();
			PreparedStatement stmt = conn.prepareStatement(sql);
			stmt.setString(1, functionNode.toString());
			...
			rs.close();
			stmt.close();
			SQLEditorInput input = new SQLEditorInput("EDIT FUNCTION ("+functionNode.toString()+").sql");
