...
import net.sourceforge.sqlexplorer.ext.mssql.utility.InfoBuilder;
...
public class UDFNode implements IDbModel {
	...
	public UDFNode(IDbModel s,String name,SQLConnection conn){
		this.conn=conn;
		parent=s;
		txt=name;
	...
	private String getSource(){
		...
		final String sql="SELECT sc.text, sc.id, sc.colid FROM " + dbName + "..sysobjects so, " + dbName + "..syscomments sc WHERE so.type='FN' AND so.name=? AND sc.id=so.id order by 2,3";
	...
	private HashMap getInfo(){
		final String dbName=parent.getParent().toString();
		final String sql="SELECT crdate FROM " + dbName + "..sysobjects WHERE type='FN' AND name=?";
