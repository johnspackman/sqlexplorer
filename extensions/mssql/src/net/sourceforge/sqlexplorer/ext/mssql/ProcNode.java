...
public class ProcNode implements IDbModel {
	...
	private String getSource(){
		...
		final String sql="SELECT sc.text, sc.id, sc.colid FROM " + dbName + "..sysobjects so, " + dbName + "..syscomments sc WHERE so.type='P' AND so.name=? AND sc.id=so.id order by 2,3";
	...
	private HashMap getInfo(){
		final String dbName=parent.getParent().toString();
		final String sql="SELECT crdate FROM " + dbName + "..sysobjects WHERE type='P' AND name=?";
