 ...
 * @author mazzolini
 */
public class SessionsNode implements IDbModel {
		...
		BusyIndicator.showWhile(display,new Runnable(){
			public void run(){
				...
				String sql="select ltrim(rtrim(loginame)) as 'User'"+
					"			,spid as 'Process ID',blocked as 'Blocked', ltrim(rtrim(sd.name)) as 'DataBase',ltrim(rtrim(sp.status)) as 'Status',cpu as 'CPU',physical_io"+
					"			as 'Physical IO',"+
	...
	public SessionsNode(IDbModel s,String name,SQLConnection conn){
		parent=s;
		txt=name;
		this.conn=conn;
	};
