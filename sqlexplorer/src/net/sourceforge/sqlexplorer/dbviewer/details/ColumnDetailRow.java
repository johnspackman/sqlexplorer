...
class ColumnDetailRow {
	...
	ColumnDetailRow(Object[] obj) throws java.lang.Exception{
		...
		el= new Object[7];
		el[0]=obj[3];//("COLUMN_NAME"); //$NON-NLS-1$
		el[1]= obj[5];//set.getString("TYPE_NAME");//map.get(new Integer(set.getShort("DATA_TYPE")));				 //$NON-NLS-1$
		//el[2]=new Integer(obj[6].toString());//new Integer(set.getInt("COLUMN_SIZE")); //$NON-NLS-1$
		el[2]=obj[6];//new Integer(set.getInt("COLUMN_SIZE")); //$NON-NLS-1$
		//el[3]=new Integer(obj[8].toString());//new Integer(set.getInt("DECIMAL_DIGITS")); //$NON-NLS-1$
		el[3]=obj[8];//new Integer(set.getInt("DECIMAL_DIGITS")); //$NON-NLS-1$
		el[6]=obj[11];//set.getString("REMARKS"); //$NON-NLS-1$
		el[4]=obj[12];//set.getString("COLUMN_DEF"); //$NON-NLS-1$
		el[5]=obj[17];//set.getString("IS_NULLABLE"); //$NON-NLS-1$
