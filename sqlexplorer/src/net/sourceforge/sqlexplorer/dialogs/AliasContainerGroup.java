	...
	public AliasContainerGroup(Composite parent, AliasModel m,DriverModel dm) {
		...
		menuMgr.add(new Action(){
			...
			public void run(){
				...
				OpenPasswordConnectDialogAction openDlgAction=
					new OpenPasswordConnectDialogAction(table.getShell(),al,driverModel,SQLExplorerPlugin.getDefault().getPreferenceStore(),SQLExplorerPlugin.getDefault().getSQLDriverManager());
				openDlgAction.run();
			}
		});
...
class AliasContentProvider implements
