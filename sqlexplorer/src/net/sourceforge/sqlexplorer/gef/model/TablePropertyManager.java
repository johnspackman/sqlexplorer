...
public class TablePropertyManager implements Serializable{
	...
	static{
		ComboBoxPropertyDescriptor iProp=new ComboBoxPropertyDescriptor(ID_SHOW_QUALIFIED,"Show Qualified Name",new String[]{"false","true"});
		ComboBoxPropertyDescriptor iProp2=new ComboBoxPropertyDescriptor(ID_SHOW_DETAILS,"Show Column Types",new String[]{"false","true"});
		descriptors = new IPropertyDescriptor[]{
			new PropertyDescriptor(ID_LABEL,"Table Name"),
			new PropertyDescriptor(ID_SCHEMA,"Schema"),
			new PropertyDescriptor(ID_CATALOG,"Catalog"),
	...
	public Object getPropertyValue(Object propName) {
		if (ID_SIZE.equals(propName))
			return new DimensionPropertySource(tb.getSize());
		else if( ID_LOCATION.equals(propName))
			return new LocationPropertySource(tb.getLocation());
		else if( ID_LABEL.equals(propName))
			//return new LabelPropertySource(label);
			return tb.getQualifiedName();
		else if( ID_SCHEMA.equals(propName))
			return tb.getSchemaName();
		else if( ID_CATALOG.equals(propName))
			return tb.getCatalogName();			
		else if (ID_SHOW_DETAILS.equals(propName))		{					
			if(tb.isShowColumnDetail())
		...
		if (ID_SIZE.equals(id)){
			...
			if(value instanceof Integer){
				if(((Integer)value).intValue()==1)
					tb.setShowColumnDetail(true);
				else
					tb.setShowColumnDetail(false);
			}
		}else if (ID_SHOW_QUALIFIED.equals(id)){
			if(value instanceof Integer){
				if(((Integer)value).intValue()==1)
					tb.setShowQualifiedName(true);
				else
					tb.setShowQualifiedName(false);
			}
		}
	}
