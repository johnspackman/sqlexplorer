...
public class SQLTableSorter extends ViewerSorter {
    ...
    public SQLTableSorter(int count, ResultSetMetaData metaData) {
        ...
        SQLType = new int[count];
        for (int i = 0; i < count; i++) {
            DEFAULT_DIRECTIONS[i] = ASCENDING;
            DEFAULT_PRIORITIES[i] = i;
            try {
                SQLType[i] = metaData.getColumnType(i + 1);
            } catch (Throwable e) {
                SQLType[i] = Types.VARCHAR;
            }
    ...
    public int compare(Viewer viewer, Object e1, Object e2) {
        return compareColumnValue((SqlRowElement) e1, (SqlRowElement) e2, 0);
    }
