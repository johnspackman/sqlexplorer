...
public class SQLTextViewer extends SourceViewer {
	...
	public SQLTextViewer(Composite parent,int style,IPreferenceStore store,final Dictionary dictionary,IVerticalRuler ruler){
		...
		//configuration.getDoubleClickStrategy(null,"").
		...
		String []indentPrefixes=(String[])vector.toArray(new String[vector.size()]);
		for(int i=0;i<contentTypes.length;i++){
			this.setIndentPrefixes(indentPrefixes,contentTypes[i]);
		}
		*/
			
		//try{
			//this.getTextHoveringController().
			//JFaceDbcPlugin.error("size "+fTextHovers.size()+" "+fTextHovers.keySet(),new Exception());
		//}catch(Exception e){};		
	...
	//public SQLTextViewer(Composite parent,int style,IPreferenceStore store){
